...
function run_lifegame_second_more($prevBoardNumber, $generation): array
{
    $b = [];
    $lns = explode('9', $prevBoardNumber);
    ...
    return [$generation, $b, $s];
}

function run_lifegame_initial(): array {
    $b = [];
    for ($i = 0; $i < 5; $i++) {
        for ($j = 0; $j < 5; $j++) {
            $b[$i][$j] = "□";
